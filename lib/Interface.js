...
module.exports = class Interface extends events.EventEmitter {
  ...
  constructor(declaredNodeClasses) {
    ...
    // We use this to handle realtime log entries AND historical
    this._mqttLogHandler = function(info) {
      const logMessage = {
        name: 'pgc_interface.pgc_interface',
        processName: 'MainProcess',
        filename: '',
      ...
      // console.log('---> Real logTopic: %s', this._logTopic);
      // this._logTopic = 'test/frontend/' +
      //   '57bd92ca6664dc16b27b8b23/logs/x0w92dqew3zr4yd00972z9phc';
      // console.log('---> Testing logTopic: %s', this._logTopic);

      _this._mqttClient.publish(this._logTopic, JSON.stringify(logMessage));
    };

    this._mqttTransport = new (logger.MqttTransport)({
      handleExceptions: true,
      level: 'debug',
      mqttHandler: this._mqttLogHandler,
    });
  ...
  async start() {
    ...
    const mqttOptions = {
      ...
      // Will send this payload if it disconnects
      will: {
    ...
    this._mqttClient.on('connect', () => {
      try {
        ...
        logger.info('topic: %s', _this._recvTopic);
  ...
  async _onMessageQueued(opt) {
    ...
    if (!this._shuttingDown) {
      ...
      switch (messageKey) {
        ...
        case 'startLogStream':
          this._startLogStream(opt.clientId); // id is not under startLogStream
          break;
  ...
  _startLogStream(clientId) {
    ...
    logger.jsonFileTransport.query(logQueryOptions, function(err, results) {
      ...
      if (err) {
        ...
        // Send historical log entries (max 100, max 24 hours), from old to new
        for (var i = results.length - 1; i >= 0; i--) {
          // We call the handler directly to send each log entries
          _this._mqttLogHandler(results[i]);
        }
      }
