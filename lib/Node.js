...
module.exports = class Node {
  constructor(nodeDefId, polyInterface, primary, address, name) {
    ...
    this.isController = false; // By default, it is false
  ...
  convertValue(driver, value) {
    ...
    switch (typeof value) {
      ...
      case 'string':
        // Empty strings are not supported by PGC
        if (!value.length) {
          return ' ';
        } else {
          return value;
      ...
      default:
        // Objects, null, undefined... return a valid empty value
        return ' ';
    }
  }
  ...
  convertDrivers() {
    ...
    Object.keys(_this.drivers).forEach(function(key) {
      _this.drivers[key].value =
        _this.convertValue(key, _this.drivers[key].value);
    });
  }
  ...
  // Sends changed drivers to ISY
  reportDrivers(forceReport = false) {
    const _this = this;
  ...
  // We receive this when a command is run.
